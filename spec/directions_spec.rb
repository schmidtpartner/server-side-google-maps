...
module ServerSideGoogleMaps
  describe(Directions) do
    ...
    describe('#initialize') do
      ...
      it('should query with :origin and :destination when given lat-lng points') do
      ...
      it('should query with :origin and :destination + waypoints when given strings') do
        Directions.should_receive(:get).with(:waypoints => 'Charlestown,MA|Lexington,MA',
                                             :origin => 'Montreal,QC', :destination => 'Ottawa,ON', :mode => :driving)
        request_params = {
            :waypoints => ['Charlestown,MA','Lexington,MA']
        }
        Directions.new('Montreal,QC', 'Ottawa,ON',request_params)
      end

      it('should query with :origin and :destination + optimized waypoints when given strings') do
        Directions.should_receive(:get).with(:waypoints => 'optimize:true|Charlestown,MA|Lexington,MA',
                                             :origin => 'Montreal,QC', :destination => 'Ottawa,ON', :mode => :driving)
        request_params = {
            :waypoints => ['Charlestown,MA','Lexington,MA'],
            :optimize => true
      ...
      it('should query with :origin and :destination + waypoints when given lat-lng points') do
        Directions.should_receive(:get).with({:waypoints=> '46.50867,-72.55368|44.4119,-76.69846',
                                              :mode=>:driving, :origin=>"45.50867,-73.55368", :destination=>"45.4119,-75.69846"})
        request_params = {
            :waypoints => [ [46.5086700,-72.5536800], [44.4119000,-76.6984600]  ]
        }
        Directions.new([45.5086700,-73.5536800], [45.4119000,-75.6984600], request_params)
      end

      it('should query with :origin and :destination + optimized waypoints when given lat-lng points') do
        Directions.should_receive(:get).with({:waypoints=> 'optimize:true|46.50867,-72.55368|44.4119,-76.69846',
                                                 :mode=>:driving,:origin=>"45.50867,-73.55368",:destination=>"45.4119,-75.69846"})
        request_params = {
            :waypoints => [ [46.5086700,-72.5536800], [44.4119000,-76.6984600]  ],
            :optimize => true
